...
foreach ( (array) $message_lines as $message_index => $message_line ) {
	echo esc_html( $message_line ) . "\n\n";

	if ( $is_paid_email && 2 === $message_index && $order instanceof WC_Order ) {
		echo esc_html__( 'Nomor Order:', 'yiari-campaign-toolkit' ) . ' ' . esc_html( $order->get_order_number() ) . "\n";
		if ( $is_shipping_package ) {
			echo esc_html__( 'Pelacakan Pesanan:', 'yiari-campaign-toolkit' ) . ' ' . esc_url( home_url( '/tracking/?order_id=' . rawurlencode( (string) $order->get_order_number() ) ) ) . "\n";
		}
		echo "\n";
	}
}
